...
function check_cli_args() {
    ...
    if [[ $action == "installFoundations" ]] || [[ $action == "installSolutions" ]] || [[ $action == "install" ]] || [[ $action == "upgradeAll" ]] || [[ $action == "upgradeFoundations" ]] || [[ $action == "upgradeSolutions" ]] || [[ $action == "validate" ]] || [[ $action == "upgradeCommonServices" ]] || [[ $action == "postUpgrade" ]]; then 
       ...
       if  [[ -z "$helm_3" ]]; then
        ...
        if [[ $cloudType != "aws" && $cloudType != "ibmcloud" && $cloudType != "azure" ]]; then
...
function check_cs_version() {
    ...
    CURRENT_CS_VERSION=$($kubernetesCLI get csv -n $CS_NAMESPACE | grep ibm-common-service | awk '{print $7;}') 
    ...
    if [[ "${CURRENT_CS_VERSION}" == "${CS_VERSION}" ]]; then
        ...
        if ! cloudctl login -a "$CS_HOST" -u "$CS_USER" -p "$CS_PASS" -n "$CS_NAMESPACE" --skip-ssl-validation; then 
            echo "Common services login isn't working"
            echo "Reinstalling common services"
            cs_version_check="true"
        fi
    elif [[ "${CURRENT_CS_VERSION}" == [3-9].[5-9].[0-9]  ]]; then
        err_exit "Newer version of Common Services already installed"
    else 
        cs_version_check="true"
    fi
        ...
        do
            ...
            if [[ -z $CS_HOST ]]; then
                if [[ $retry -eq ${maxRetry} ]]; then 
                    err_exit "Failed to retrieve Common Services cp-console route"
                else
                    sleep 60
                    continue
                fi
...
function install_cs(){  
    ...
    if [[ "$cs_version_check" == "true" ]]; then
        ...
        if [ "X$airgapInstall" != "X" ]; then
            run "$cs_install_file -cp4sns $namespace -airgap $repo" "Common Services Install"
        else
            run "$cs_install_file -cp4sns $namespace" "Common Services Install"
        fi
    fi
}
...
function upgrade_charts(){
  ...
  if [[ $chart == "ibm-security-foundations" ]]; then
     ...
     if [ -z "$isPresen" ]; then
         ...
         deployed_version=$($kubernetesCLI get deploy middleware -o jsonpath='{.spec.template.metadata.annotations.cloudpakVersion}')
     ...
     if ! ${helm3} upgrade --install "$foundations_release_name" --namespace="$namespace" --set global.cloudType="${cloudType}" --set global.repositoryType=${repositoryType} $pullPolicy --set global.repository="${chart_url}" --set global.license="$license" --set global.helmUser="${CS_USER}" --values "${chartsDir}"/ibm-security-foundations-prod/values.yaml "${chartsDir}"/ibm-security-foundations-prod --reset-values $force --timeout 1000s; then
        ...
        if [ -z "$isPresen" ]; then
           ...
           force="--force" 
            ...
            deployed_version=$($kubernetesCLI get deploy inf-configstore -o jsonpath='{.spec.template.metadata.annotations.cloudpakVersion}')
...
function upgrade_cs(){
    ...
    if [ "X$airgapInstall" != "X" ]; then
        run "$upgrade_file -cp4sns $namespace -airgap $repo" "Common Services Upgrade"
    else
        run "$upgrade_file -cp4sns $namespace" "Common Services Upgrade"
    fi    
}
    ...
    if [[ $flag == "solutions" ]]; then
        ...
        if ! $kubernetesCLI get namespace ibm-common-services >/dev/null 2>&1; then
            ...
            if [[ -z "$isPresent" ]] || [[ -z "$csPods" ]]; then
                ...
                check_cs_version
                if [[ "${cs_version_check}" == "true" ]]; then
                    run "$scriptDir/support/uninstall.sh -uninstall_cs case ${repo}" "Common Services uninstall"
                fi
           fi
